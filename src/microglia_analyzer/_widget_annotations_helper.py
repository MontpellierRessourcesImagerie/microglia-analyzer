...
Any layer starting with this prefix will be considered as a class.
The name of the class is the suffix of the layer name.
"""
_CLASS_PREFIX = "class."

"""
Name of the layer containing the current image.
This will be the backdrop for adding YOLO annotations or draw masks.
"""
_IMAGE_LAYER  = "µ-glia-image"
...
We want to be able to read both TIFF and PNG/JPG images.
skimage is very bad at reading correctly TIFF images, and tifffile can't open PNG/JPG.
So we use tifffile for TIFF and skimage for PNG/JPG.
"""
imread = tifffile.imread

"""
Indices to have the width and height from an image shape.
Depending on the library that reads the image, the order of the dimensions can change.
This tuple is used to get the right dimensions of the image.
"""
_WIDTH_HEIGHT = (0, 2)

"""
Arguments to pass to the imread function.
The imread of skimage opens images as RGB by default, even if they are grayscale.
So we used the 'as_gray' argument to force the image to be read as grayscale.
"""
ARGS = {}
...
class AnnotationsWidget(QWidget):
    ...
    def __init__(self, napari_viewer):
        ...
        # Folder in which are located the 'images', 'annotations' and potentially 'masks' folders.
        ...
        # List of images ('.tif', '.png', ...) in the 'images' folder.
        self.images_list = []
    ...
    def add_annotations_management_group_ui(self):
        box = QGroupBox("YOLO annotations")
        layout = QVBoxLayout()
        box.setLayout(layout)
    ...
    def is_image_opened(self):
        ...
        """
        return (_IMAGE_LAYER in self.viewer.layers) and \
               (self.viewer.layers[_IMAGE_LAYER].data is not None)
    ...
    def layer2yolo(self, layer_name, index):
        """
        Produces a list of tuples containing the bounding-boxes of a layer in YOLO format.
        This list is only for one class (one layer).
        Converts the bounding-boxes from the Napari format to the YOLO format.

        Args:
            - layer_name (str): The name of the layer to convert.
            - index (int): The class index of the layer.
        
        Returns:
            (list): A list of tuples, each tuple containing the class index and the YOLO bounding-box.
        """
    ...
    def update_reader_fx(self):
        """
        Uses the first element of the images list to determine which library to use to read images.
        In consequences:
            - The imread function is set to the right library.
            - The indices to get the width and height of an image are set.
            - The arguments to pass to the imread function are set.
        """
        global imread
